...
<?php } }?>
<p><a href="<?php echo get_option('siteurl').'/admin/admin.php?page=affiliates_settings&pagetype=addedit'?>"><strong><?php _e('Add Affiliate Link'); ?></strong></a> </p>
...
if($affiliate_links)
{
	...
	?>
    <tr>
      <td><a href="<?php echo get_option( 'siteurl' );?>/admin/admin.php?page=affiliates_settings&pagetype=addedit&lid=<?php echo $key;?>" title="<?php _e('Edit');?> <?php echo $affiliate_links_Obj['link_title'];?>"><?php echo $affiliate_links_Obj['link_title'];?></a></td>
	  ...
	  <?php 
		...
		{
			echo '<a href="'.get_option( 'siteurl' ).'/admin/admin.php?page=affiliates_settings&status=0&lid='.$key.'">'.__('Hide').'</a>';
		}else
		{
			echo '<a href="'.get_option( 'siteurl' ).'/admin/admin.php?page=affiliates_settings&status=1&lid='.$key.'">'.__('Show').'</a>';
		}
		?>
        |
       <a href="<?php echo get_option( 'siteurl' );?>/admin/admin.php?page=affiliates_settings&act=delete&lid=<?php echo $key;?>" onclick="return confirm_del();"> <?php _e('Delete');?></a>
      </td>
    </tr>
    <?php
    }
