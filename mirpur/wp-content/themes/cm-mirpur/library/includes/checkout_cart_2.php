<script>
 function setAttributeVal2()
 {
	...
	if(postformflag)
	{
		...
		{
			document.getElementById('addtocartformspan2').innerHTML = '<?php _e('processing ...');?>';
		}
...
function postform1()
{
	...
	$.ajax({
		...
		success: function(html){
			...
			if(eval(document.getElementById('addtocartformspan2')))
			{
				document.getElementById('addtocartformspan2').innerHTML = '<strong><?php _e('Added in Cart');?><Br><a href="<?php echo get_option('siteurl'); ?>/?page=cart"><?php _e('View Cart Detail');?></a> or <a href="<?php echo get_option('siteurl'); ?>/?page=cart"><?php _e('Checkout &raquo;');?> &raquo;</a></strong>';
			}
		}
	});
	return false;
...
<strong><?php _e('Qty:');?></strong>  <input name="product_qty" id="product_qty2" type="text" onKeyPress="return numberonly(event)" value="1" class="textbox" />
  </div>
