<?php
global $wpdb,$General,$upload_folder_path;
if($_POST)
{
	...
	if($cartinfo)
	{
		foreach($cartinfo as $cartinfoObj)
		{
			...
			$is_on_ssl = 	$option_value['is_on_ssl'];
	...
	update_option('shoppingcart_general_settings',$option_value);
...
<form action="<?php echo get_option( 'siteurl' );?>/admin/admin.php?page=settings" method="post">
  <style>
...
>
      ...
      <tr>
        <td><?php _e('Default Currency (Ex.: PKR)'); ?></td>
        <td><input type="text" name="currency" value="<?php echo $currency;?>" /></td>
      </tr>
      <tr>
        <td><?php _e('Default Currency Symbol (Ex.: Rs.)'); ?></td>
      ...
      <tr>
        <td><?php _e('Product Image Path'); ?> (<?php echo get_option('siteurl') . "$upload_folder_path";?>) <br />
          <?php _e('(Default folder is "products_img")'); ?> </td>
        <td><input type="text" name="imagepath" value="<?php echo $imagepath;?>" /></td>
      </tr>
      <tr>
        <td><?php _e('Digital Product Path'); ?> (<?php echo get_option('siteurl') . "$upload_folder_path";?>) <br />
      ...
      <tr>
        <td><?php _e('Display "Related Posts" on Product detail page'); ?></td>
	  ...
	  <tr><td colspan="2"><h2><?php _e('Stock Settings'); ?></h2></td></tr>
       ...
       <tr>
        <td><?php _e('Enable SSL - https on checkout page'); ?></td>
        <td><select name="is_on_ssl">
            <option value="1" <?php if($is_on_ssl==1){?> selected="selected"<?php }?>>Yes</option>
            <option value="0" <?php if($is_on_ssl=='0'){?> selected="selected"<?php }?>>No</option>
          </select>        </td>
      </tr>
      <tr>
     ...
     <?php /*?> <tr>
      ...
      <tr>
        <td><?php _e('Display Number of Orders on Admin Designer Club'); ?></td>
       ...
       ( <?php _e('Display Term and Conditions checkbox at checkout page'); ?>)        </td>
        <td><select name="is_show_termcondition">
          ...
          syntex beside checkbox <textarea name="termcondition" cols="50" rows="1"><?php echo stripcslashes($termcondition);?></textarea></td>
      </tr>
       <tr>
