...
if($_POST['frm_action'] == 'addeditlinks')
{
	...
	if($link_title)
	{
		...
		$location = get_option('siteurl')."/admin/admin.php?page=affiliates_settings&msg=success";
...
<form action="<?php echo get_option('siteurl')?>/admin/admin.php?page=affiliates_settings&pagetype=addedit&lid=<?php echo $_REQUEST['lid'];?>" method="post" name="coupon_frm">
  ...
  <table width="75%" cellpadding="3" cellspacing="3" class="widefat post fixed" >
    ...
    <tr>
      ...
      <td><input type="submit" name="submit" value="<?php _e('Submit');?>" onclick="return check_frm();" class="button-secondary action" >
        &nbsp;
        <input type="button" name="cancel" value="<?php _e('Cancel');?>" onClick="window.location.href='<?php echo get_option('siteurl')?>/admin/admin.php?page=affiliates_settings'" class="button-secondary action" ></td>
    </tr>
  </table>
</form>
<script>
