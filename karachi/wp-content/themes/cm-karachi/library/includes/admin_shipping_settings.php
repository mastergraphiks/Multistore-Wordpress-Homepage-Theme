...
if($_POST)
{
	...
	wp_redirect(get_option( 'siteurl' )."/admin/admin.php?page=manageshipping&payact=setting&msg=success&id=".$_GET['id']);
}
...
<form action="<?php echo get_option( 'siteurl' );?>/admin/admin.php?page=manageshipping&payact=setting&id=<?php echo $_GET['id'];?>" method="post" name="payoptsetting_frm">
  <style>
...
?>
      <tr>
        ...
        <td><input type="submit" name="submit" value="<?php _e('Submit'); ?>" class="button-secondary action" />
          &nbsp;
          <input type="button" name="cancel" value="<?php _e('Cancel'); ?>" onClick="window.location.href='<?php echo get_option( 'siteurl' )."/admin/admin.php?page=manageshipping"; ?>'" class="button-secondary action" /></td>
        <td>&nbsp;</td>
      </tr>
    </thead>
  </table>
