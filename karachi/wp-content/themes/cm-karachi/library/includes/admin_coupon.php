...
if($_POST['couponact'] == 'addcoupon')
{
	...
	if($couponcode)
	{
		...
		if($couponinfo)
		{
			...
			}else
			{
				for($i=0;$i<count($option_value);$i++)
				{
					...
					{
						$location = get_option('siteurl')."/admin/admin.php?page=managecoupon&pagetype=addedit&msg=exist";
		...
		$location = get_option('siteurl')."/admin/admin.php?page=managecoupon&msg=success";
...
<form action="<?php echo get_option('siteurl')?>/admin/admin.php?page=managecoupon&pagetype=addedit&code=<?php echo $_REQUEST['code'];?>" method="post" name="coupon_frm">
...
--></td>
    ...
    <tr>
      ...
      <td><input type="submit" name="submit" value="<?php _e('Submit');?>" onclick="return check_frm();" class="button-secondary action" >
        &nbsp;
        <input type="button" name="cancel" value="<?php _e('Cancel');?>" onClick="window.location.href='<?php echo get_option('siteurl')?>/admin/admin.php?page=managecoupon'" class="button-secondary action" ></td>
    </tr>
  </table>
</form>
<script>
