 ...
 <?php _e('Search By');?> : 
    ...
    &nbsp;<img src="<?php echo bloginfo('template_directory');?>/images/cal.gif" alt="Calendar" onclick="displayCalendar(document.frm_srch.srch_end_date,'yyyy-mm-dd',this)" style="cursor: pointer;" align="absmiddle" border="0">
     <input type="submit" name="submit" value="<?php _e('Search');?>" class="highlight_input_btn" />
      <input type="button" name="default" onclick="window.location.href='<?php echo get_option('siteurl');?>/admin/admin.php?page=affiliate_report&user_id=<?php echo $userId;?>'" value="<?php _e('Default');?>" class="highlight_input_btn" />
     <?php
     $params = '';
...
if($user_affiliate_data)
{
	...
	foreach($user_affiliate_data as $key => $val)
	{
		...
		?>
			...
			<td class="row1" ><?php echo number_format($order_amount,2);?></td>
...
{
?>
<tr><td colspan="6">&nbsp;</td><th><?php _e('Total Earn Amount');?> </th><th><?php echo number_format($share_total,2);?></th></tr>
<?php
}
	?>  </thead>
</table>
