...
if($_POST['submitact']=='delete')
{
	...
	wp_redirect(get_option('siteurl')."/admin/admin.php?page=manageorders&msg=delete");
}
?>
<script>
...
<table width="100%">
  <tr>
    <td><form method="post" action="<?php echo get_option('siteurl');?>/admin/admin.php?page=manageorders" name="ordersearch_frm">
...
?>
            ...
            <td valign="top"><strong><?php _e('Order Status'); ?> :</strong>
              <select name="srch_status">
                ...
                <option value="processing" <?php if($_REQUEST['srch_status']=='processing'){?> selected<?php }?>><?php _e('Processing')?></option>
                <option value="approve" <?php if($_REQUEST['srch_status']=='approve'){?> selected<?php }?>><?php _e('Approved');?></option>
                <option value="reject" <?php if($_REQUEST['srch_status']=='reject'){?> selected<?php }?>><?php _e('Rejecte');?></option>
                <option value="cancel" <?php if($_REQUEST['srch_status']=='cancel'){?> selected<?php }?>><?php _e('Canceled');?></option>
                <option value="shipping" <?php if($_REQUEST['srch_status']=='shipping'){?> selected<?php }?>><?php _e('Shipping');?></option>
                <option value="delivered" <?php if($_REQUEST['srch_status']=='delivered'){?> selected<?php }?>><?php _e('Delivered');?></option>
              </select></td>
            <td valign="top">&nbsp;&nbsp;
...
function chkfrm()
{
	if(document.getElementById('srch_orderno').value=='order number')
	{
		document.getElementById('srch_orderno').value = '';
            ...
            <td valign="top"><input type="button" name="Default" value="<?php _e('List All Orders'); ?>" onclick="window.location.href='<?php echo get_option('siteurl');?>/admin/admin.php?page=manageorders'" class="button-secondary action" /></td>
          </tr>
          <tr>
      ...
      <input type="hidden" name="submitact" value="delete" />
        <table width="100%" cellpadding="5"  class="widefat post fixed" >
          <thead>
            ...
            <?php
$targetpage = get_option('siteurl').'/admin/admin.php?page=manageorders';
$recordsperpage = 30;
...
?>
            <tr>
              ...
              <td><a href="<?php echo get_option('siteurl');?>/admin/admin.php?page=manageorders&oid=<?php echo $order_info['order_id'];?>"><?php echo $order_info['order_id'];?></a></td>
              <td><a href="<?php echo get_option('siteurl');?>/admin/user-edit.php?user_id=<?php echo $userId;?>"><?php echo $user_info['user_name'];?></a></td>
