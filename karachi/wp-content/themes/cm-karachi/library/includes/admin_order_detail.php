...
if($_POST['act']) // update order 
{
	...
	if($issendEmail)
	{
		...
		{
			$admindestinationfile =   ABSPATH . $upload_folder_path."notification/emails/order_approval.txt";
		...
		{
			$admindestinationfile =   ABSPATH . $upload_folder_path."notification/emails/order_rejection.txt";
		...
		if($General->is_active_affiliate())
		{
			if($_POST['ostatus']=='approve')  // send affiliate email
			{
				...
				if($aid)
				{
					...
					$order_amount = $cart_amt - $order_info_all['discount_amt'];
					...
					<p>Order Amount :       '.number_format($order_amount,2).'</p>
					<p>Qty :       '.$product_qty.'</p>
...
<tr>
  <td><table width="100%">
      ...
      <tr>
        <td><form action="<?php echo get_option( 'siteurl' ) ."/admin/admin.php?page=manageorders&oid=".$_GET['oid'];?>" method="post">
      ...
      <tr>
        <td><a href="<?php echo get_option('siteurl')."/admin/admin.php?page=manageorders"?>"><strong><?php _e('Back to Orders Listing'); ?></strong></a></td>
      </tr>
      <tr>
        <td>&nbsp;</td>
      </tr>
