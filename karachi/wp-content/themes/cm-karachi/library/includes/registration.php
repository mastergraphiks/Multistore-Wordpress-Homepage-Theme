 ...
 ?>
   <div class="checkout_r" style="width:28%" >
    ...
    <p> <?php _e(GUEST_CHECKOUT_MSG);?>  </p>
     <input class="common_button " type="button" onclick="window.location.href='<?php global $General; echo $General->get_ssl_normal_url(get_option('siteurl'));?>/?page=checkout&checkout_as_guest=1'" value="Checkout &raquo;" name="confirm"/>
   </div>
 <?php
 }
 ?>
...
default:
	...
	if ( !is_wp_error($user) ) {
		...
		if ( !$user->has_cap('edit_posts') && ( empty( $redirect_to ) || $redirect_to == 'admin/' || $redirect_to == admin_url() ) )
...
?>
		...
		{
			global $General;
			$login_redirect_link = $General->get_ssl_normal_url(get_option('siteurl')).'/?page=cart';
