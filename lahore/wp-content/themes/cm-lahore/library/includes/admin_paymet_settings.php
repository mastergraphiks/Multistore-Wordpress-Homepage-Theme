...
if($_POST)
{
	...
	wp_redirect(get_option( 'siteurl' )."/admin/admin.php?page=paymentoptions&payact=setting&msg=success&id=".$_GET['id']);
	exit;
}
if($_GET['status']!= '')
{
...
<form action="<?php echo get_option( 'siteurl' );?>/admin/admin.php?page=paymentoptions&payact=setting&id=<?php echo $_GET['id'];?>" method="post" name="payoptsetting_frm">
  <style>
...
?>
      <tr>
        ...
        <td><input type="submit" name="submit" value="<?php _e('Submit'); ?>" onclick="return chk_form();" class="button-secondary action" />
          &nbsp;
          <input type="button" name="cancel" value="<?php _e('Cancel'); ?>" onclick="window.location.href='<?php echo get_option( 'siteurl' )."/admin/admin.php?page=paymentoptions"; ?>'" class="button-secondary action" /></td>
      </tr>
    </thead>
  </table>
</form>
